...
if(isset($_GET["action"])&&($_GET["action"]=="delete")){
	...
	$stmt->close();
	//重新導向回到主畫面
	header("Location: member_prove.php");
}
//核准會員資料
if(isset($_GET["action"])&&($_GET["action"]=="confirm")){
  $query_delMember = "UPDATE `memberdata` SET `m_statue` = 'true' WHERE m_id=?";
  $stmt=$db_link->prepare($query_delMember);
  $stmt->bind_param("i", $_GET["id"]);
  $stmt->execute();
  $stmt->close();
...
//未加限制顯示筆數的SQL敘述句
$query_RecMember = "SELECT * FROM memberdata WHERE m_statue ='false' ORDER BY m_jointime DESC";
        ...
        <td class="tdrline"><p class="title">會員核准列表 </p>
          <table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#F0F0F0">
            <tr>
              ...
              <th width="20%" bgcolor="#CCCCCC"><p>信箱</p></th>
              <th width="20%" bgcolor="#CCCCCC"><p>電話</p></th>
              <th width="10%" bgcolor="#CCCCCC"><p>加入時間</p></th>
            </tr>
			<?php while($row_RecMember=$RecMember->fetch_assoc()){ ?>
            <tr>
              <td width="10%" align="center" bgcolor="#FFFFFF"><p><a href="?action=confirm&id=<?php echo $row_RecMember["m_id"];?>" onClick="return confirmsure();">核准</a><br>
                <a href="?action=delete&id=<?php echo $row_RecMember["m_id"];?>" onClick="return deletesure();">刪除</a></p></td>
              <td width="15%" align="center" bgcolor="#FFFFFF"><p><?php echo $row_RecMember["m_name"];?></p></td>
              <td width="15%" align="center" bgcolor="#FFFFFF"><p><?php echo $row_RecMember["m_username"];?></p></td>
              <td width="15%" align="center" bgcolor="#FFFFFF"><p><?php echo $row_RecMember["m_email"];?></p></td>
              <td width="15%" align="center" bgcolor="#FFFFFF"><p><?php echo $row_RecMember["m_phone"];?></p></td>
              <td width="15%" align="center" bgcolor="#FFFFFF"><p><?php echo $row_RecMember["m_jointime"];?></p></td>
            </tr>
			<?php }?>
          </table>          
          <hr size="1" />
